...
def action_input_map():
    input_map = {
        ...
        'return_tool': 'return_toolreturn_tool'
    }
    return input_map
...
def add_tool(args, userid):
    ...
    try:
      ...
      ('{0}','{1}','{2}','{3}','{4}','{5}','{6}','{7}');".format(0, random.randint(0, 999999999), name, desc, sharable,
                                                                 price, d, available))
...
def update_tool(args, userid):
    ...
    if tool_exists:
        try:
            ...
            tool = exec_get_one(
                "SELECT tool_id,times_lent,barcode,name,description,shareable,purchase_price,purchase_date,available from tools where tool_id='{0}';".format(
                    args[1]))
            print_tuple(["tool_id", "times_lent", "barcode", "name", "description", "shareable", "purchase_price",
                         "purchase_date", "available"], tool)

            cols = ["times_lent", "barcode", "name", "description", "shareable", "purchase_price", "purchase_date",
                    "available"]
...
def search_tool(args, userid):
    ...
    if args[1].lower() == "name":
        cols = ["tool_id", "name", "description"]
        tuples = exec_get_all(
            "SELECT tool_id, name, description from tools where name like '%%_{0}%%'".format(search_terms[0][1:]))
        print_tuples(cols, tuples)

    elif args[1].lower() == "category":
        category_tup = exec_get_all(
            "SELECT category_id, name from categories where name like '%%_{0}%%'".format(args[2]))
        print("Getting tools in category {0}".format(category_tup[0][1]))
        tools_in_category = exec_get_all(
            "SELECT tool_id from categorized_tools where category_id = '{0}'".format(category_tup[0][0]))
        for tool in tools_in_category:
            tuples.append(
                exec_get_one("SELECT tool_id, name, description from tools where tool_id = '{0}'".format(int(tool[0]))))
        ...
        cols = ["tool_id", "name", "description"]
        tuples = exec_get_all(
            "SELECT tool_id, name, description from tools where barcode = '{0}'".format(search_terms[0]))
        print_tuples(cols, tuples)
...
def return_tool(args, userid):
    ...
    if tool_exists:
        try:
            exec_commit("UPDATE user_tool_requests SET date_returned = '{0}' WHERE tool_id = "
                        "'{1}' and requesting_user_id = '{2}';".format(d, toolid, userid))
            print("Tool Returned!")
        except Exception as e:

...
def inspect_lent_tools(args):
    ...
    tuples = exec_get_all(
        ...
        "WHERE r.status ='accepted' AND r.date_borrowed is not null AND date_returned is null "  
        "ORDER BY r.date_borrowed ASC;")
    print('Total found:', len(tuples))
    ...
    for tup in tuples:
        ...
        print(
            "Tool:{0} | holder:{1} | overdue:{2} | date borrowed:{3}".format(tup[1], tup[2],
                                                                                                 str(tup[3]),
                                                                                                 str(tup[4]),
                                                                                                 str(tup[5])))
...
def inspect_borrowed_tools(args):
    ...
    tuples = exec_get_all("SELECT t.name, u.username as tool_owner, r.overdue, r.date_borrowed "
                          "FROM user_tool_requests r "
                          ...
                          "ORDER BY r.date_borrowed ASC;")
    print('Total found:', len(tuples))
    [print("Tool:{0} | owner:{1} | overdue:{2} | date borrowed:{3} ".format(t[0], t[1], str(t[2]), str(t[3]))) for t in
     tuples]